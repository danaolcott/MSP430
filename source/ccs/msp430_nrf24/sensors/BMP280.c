...
void BMP280_init(void)
{
	...
	//Load Calibration data into ram - mCalibrationData
...
BMP280_Data BMP280_read(void)
{
	...
	//correct values using calibration data
	...
	result.cTemperatureCInt = (uint8_t)(cTemperature / 100);
	result.cTemperatureCFrac = (uint8_t)(cTemperature % 100);

	BMP280_S32_t cTemperatureF = cTemperature * 9 / 5;
	cTemperatureF += 3200;

	result.cTemperatureFInt = (uint8_t)(cTemperatureF / 100);
	result.cTemperatureFFrac = (uint8_t)(cTemperatureF % 100);
