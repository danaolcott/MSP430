...
//
//P1.4 - IRQ Pin - Configured in main
//P2.0 - CE Pin - output
//
//Pass the final mode
...
void nrf24_writeTXPayLoad(uint8_t* buffer, uint8_t length)
{
    ...
    if (numBytes != NRF24_PIPE_WIDTH)
        numBytes = NRF24_PIPE_WIDTH;
...
void nrf24_transmitData(uint8_t* buffer, uint8_t length)
{
    ...
    {
        usart_writeString("Timeout - Counter Expired - Transmit Aborted\r\n");
    ...
    {
        usart_writeString("Polling - Transmit Complete: ");
        usart_writeStringLength(buffer, length);
        usart_writeString("\r\n");
    }
    
    else
    {
    	usart_writeString("Polling - Transmit Flag Never Set - Timeout\r\n");
    }
    
    /////////////////////////////////////////
    //Test the tx_ds and max_rt bits
    if (nrf24_getStatus() & (NRF24_BIT_TX_DS | NRF24_BIT_MAX_RT ))
    {
        //tx ds bit high - clearing it
    	usart_writeString("TX_DS and or MAX_RT Bit(s) High - Clearing It...\r\n");
        nrf24_writeReg(NRF24_REG_STATUS, NRF24_BIT_TX_DS | NRF24_BIT_MAX_RT);
    }   
}
...
//Mapped to pin P1.4 - input, falling edge
...
void nrf24_ISR(void)
{
    ...
    if (status & NRF24_BIT_RX_DR)
    {
        ...
        while (nrf24_RxFifoHasData())
        {
            ...
            usart_writeStringLength(rxBuffer, len);               //forward it to the uart
            usart_writeString("\r\n");
    ...
    {
    	usart_writeString("Max Retries Failed...\n");
        nrf24_writeReg(NRF24_REG_STATUS, NRF24_BIT_TX_DS | NRF24_BIT_MAX_RT);
    }    
}
